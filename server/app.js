// app.js — builds the Express app (routes only). Exported so tests can import it
// without starting a server or touching a real database.
  ...
  app.get("/api/entries", requireAuth, h(async (req, res) => {
    ...
    const { rows } = await pool.query(
      `SELECT id, to_char(day,'YYYY-MM-DD') AS day, category, item, amount, co2e
         FROM entries WHERE user_id=$1 AND day BETWEEN $2 AND $3 ORDER BY day DESC, created_at DESC`,
      [req.userId, from, to]);
...
export function serializeUser(u) { return { id: u.id, email: u.email, name: u.name, region: u.region, theme: u.theme, goal: u.goal_kg }; }
...
export function isoDate(v) { return /^\d{4}-\d{2}-\d{2}$/.test(v || "") ? v : null; }
